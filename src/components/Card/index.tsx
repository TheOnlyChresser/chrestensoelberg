      ...
      <div className="p-4">
        {titleToUse && (
          <div className="prose font-bold">
            <h3>
              <Link className="-m-4 font-inter no-underline break-all not-prose text-black dark:text-white group-hover:text-white dark:group-hover:text-white dark:group-active:text-black group-active:text-white line-clamp-3 md:line-clamp-2 p-1" href={href} ref={link.ref}>
                {titleToUse}
              </Link>
            </h3>
          </div>
                ...
                {categories?.map((category, index) => {
                  if (typeof category === 'object') {
                    ...
                    return (
                        <span
                            key={index}
                            className="font-inter bg-blue-500/10 dark:bg-blue-400/20 dark:border-white/20 group-hover:bg-white/10 dark:text-white/80 group-active:bg-white/10 border border-black/20 rounded-full px-3 py-1 text-xs uppercase italic"
                        >
                        {categoryTitle}
                      </span>
                    )}
