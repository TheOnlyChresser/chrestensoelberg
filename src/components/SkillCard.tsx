...
export default function SkillCard({name, icon, website, description}: SkillCardProps) {
    ...
    return (
        <>
            <div 
                onClick={handleClick}
                className="cursor-pointer flex-center space-x-2 p-4 bg-white/25 rounded-xl shadow-md hover:shadow-lg active:shadow-lg group hover:scale-110 active:scale-110 transition-all duration-200 delay-[-200ms] md:delay-[0ms]"
            >
            ...
            {isPopupOpen && mounted && createPortal(
                ...
                >
                    <div 
                        className="bg-white/95 backdrop-blur-xl rounded-2xl p-6 max-w-md w-full shadow-2xl border border-white/20 transform transition-all duration-200"
