...
export function PriceBlockWrappper({ children, className = "", ...props }: PriceBlockWrapperProps) {
    return (
        <div className={`group flex flex-col rounded-xl bg-white w-full md:w-[28vw] min-h-[40vh] md:min-h-[65vh] border border-gray-200 shadow-lg items-center justify-between p-6 transition-transform hover:scale-105 hover:shadow-xl ${className}`} {...props}>
            {children}
        </div>
    )
}
...
export function PriceBlockTitle({children, className = "", ...props}: PriceBlockTitleProps) {
    return (
        <div className={`w-full text-3xl font-semibold text-center text-gray-800 mb-0 ${className}`} {...props}>
            {children}
        </div>
    )
}
...
export function PriceBlockPrice({ children, currency = "kr", className = "", ...props }: PriceBlockPriceProps) {
    return (
        <div className={`text-center text-xl font-bold text-gray-900 mt-0 mb-1 ${className}`} {...props}>
            {children}{currency}
        </div>
    )
}
...
export function PriceBlockUndertitle({children, className = "", ...props}: PriceBlockUndertitleProps) {
    return (
        <p className={`text-center text-gray-500 text-sm ${className}`} {...props}>
            {children}
        </p>
    )
}
...
export function PriceBlockExtra({children, className = "", ...props}: PriceBlockWrapperProps) {
    return (
        <div className={`flex items-center justify-center ${className}`} {...props}>
            <span className="cursor-pointer bg-gradient-to-r from-blue-100 via-sky-100 to-cyan-100 text-sky-900 border border-sky-400 px-4 py-1 rounded-md text-sm md:text-base font-semibold shadow-sm tracking-wide animate-pulse text-center group-hover:animate-none text-wrap">{children}</span>
        </div>
    )
}